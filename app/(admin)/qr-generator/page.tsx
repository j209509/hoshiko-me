      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div className="space-y-5">
                ...
                <div className="flex gap-2">
                  {colorThemes.map((t, i) => (
                    <button key={t.name} onClick={() => setThemeIndex(i)}
                      className={`w-8 h-8 rounded-full transition-all ${themeIndex === i ? "ring-2 ring-offset-2 ring-gray-400 scale-110" : "hover:scale-105"}`}
          ...
          <div className="flex gap-3">
            <Button onClick={() => window.print()} className="flex-1 gap-2" style={{ backgroundColor: theme.bg }}>
        ...
        <div>
          ...
          <div className="border border-gray-200 rounded-xl overflow-hidden shadow-md">
            <div ref={posterRef} className="aspect-[210/297] relative flex flex-col" style={{ backgroundColor: theme.bg }}>
              ...
              <div className="relative z-10 flex flex-col h-full p-8">
                <div className="text-center mb-6">
                  <div className="text-xs font-medium mb-2 opacity-80" style={{ color: theme.accent }}>
                    ご来店ありがとうございます
                  </div>
                ...
                <div className="flex justify-center mb-6">
                  <div className="bg-white p-4 rounded-2xl shadow-lg">
                    {reviewUrl && <QRCode value={reviewUrl} size={160} level="M" fgColor="#1e1e2e" />}
                  </div>
                </div>

                {displayIncentive && (
                  <div className="text-center py-4 px-6 rounded-2xl mb-4"
                    style={{ backgroundColor: "rgba(255,255,255,0.15)" }}>
                    <div className="text-xs opacity-80 mb-1" style={{ color: theme.text }}>レビューご記入で</div>
                    <div className="text-xl font-bold" style={{ color: theme.text }}>🎁 {displayIncentive}</div>
                    <div className="text-xs mt-1 opacity-70" style={{ color: theme.text }}>※スタッフにお声がけください</div>
                  </div>
                )}
                  ...
                  <div className="flex justify-center gap-4">
                    {["スキャン", "評価入力", "完了！"].map((step, i) => (
                      <div key={step} className="flex items-center gap-2">
                        ...
                        <span className="text-xs" style={{ color: theme.text, opacity: 0.9 }}>{step}</span>
                        {i < 2 && <span className="text-xs opacity-50" style={{ color: theme.text }}>›</span>}
                      </div>
                    ))}
                  </div>
                </div>
